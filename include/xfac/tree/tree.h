...
//---  function for tree validation

/// True if every node in neigh is reachable from the first node (Breadth-First Search  BFS).
inline bool isConnected(TopologyTree const& tree)
{
    if (tree.neigh.empty()) return true;
    std::set<int> visited;
    std::queue<int> q;
    int start = tree.neigh.begin()->first;
    q.push(start); visited.insert(start);
    while (!q.empty()) {
        int cur = q.front(); q.pop();
        for (int nb : tree.neigh.at(cur).from_int())
            if (!visited.count(nb)) { visited.insert(nb); q.push(nb); }
    }
    return visited.size() == tree.neigh.size();
}

/// True if neigh contains no cycle (BFS back-edge detection).
inline bool isTree(TopologyTree const& tree)
{
    if (tree.neigh.empty()) return true;
    ...
    while (!q.empty()) {
        ...
        for (int nb : tree.neigh.at(cur).from_int()) {
            ...
            else if (nb != parent[cur]) return false;
        }
    }
    return true;
}

/// True if every physical node in nodes appears as a key in neigh.
inline bool areNodesInNeigh(TopologyTree const& tree)
{
    for (int node : tree.nodes)
        if (!tree.neigh.count(node)) return false;
    return true;
}

/// True if root is a key in neigh.
inline bool isRootInNeigh(TopologyTree const& tree)
{
    return tree.neigh.count(tree.root) > 0;
}

/// True if every neighbor listed in neigh is itself a key in neigh (ignoring dummy nodes).
inline bool areNeighborsDefined(TopologyTree const& tree)
{
    for (auto& [node, neighbors] : tree.neigh)
        for (int nb : neighbors.from_int())
            if (nb != -1 && !tree.neigh.count(nb)) return false;
    return true;
}

/// True if for every neighbor B of A, A is also a neighbor of B (ignoring dummy nodes).
inline bool areEdgesSymmetric(TopologyTree const& tree)
{
    for (auto& [node, neighbors] : tree.neigh)
        for (int nb : neighbors.from_int())
            if (nb != -1 && !tree.neigh.at(nb).to_int().count(node)) return false;
    return true;
}

/// Validates all structural invariants of the tree, reporting failures if verbose=true.
/// Returns true only if all checks pass. areNeighborsDefined is checked first since
/// other checks may throw if it fails.
inline bool isValid(TopologyTree const& tree, bool verbose = false)
{
    bool valid = true;

    auto check = [&](bool result, std::string const& name) {
        if (!result) {
            if (verbose) std::cout << "isValid: failed " << name << "\n";
            valid = false;
        }
        return result;
    };

    // check first: other BFS checks may throw if neighbors are undefined
    if (!check(areNeighborsDefined(tree), "areNeighborsDefined")) return false;

    check(areEdgesSymmetric(tree),        "areEdgesSymmetric");
    check(isConnected(tree),              "isConnected");
    check(isTree(tree),                   "isTree");
    check(areNodesInNeigh(tree),          "areNodesInNeigh");
    check(isRootInNeigh(tree),            "isRootInNeigh");
